...
class UploadFormatter:
    ...
    def main(self):
        """Main function"""
        _quiz_path = self.USER_CHECK # "..\\4_HUMAN_CHECK\\human_check.json" # look for human - approved questions, and store all in archive, for training data
        _raw = self.read_json(_quiz_path)
        c_quiz = ""
        for _q_idx, _q_items in _raw.items():
            _header = self.make_quiz_format_head(folder=self._folder, category=self._category, quiz=_q_items, shuffle=True, direction="column")
        ...
        if c_quiz[-3:] == '\n-\n':
            c_quiz = c_quiz[:-3]
        print(c_quiz)
        self.save_quiz_format(c_quiz, self.TESTINVITE) # "..\\5_FINAL_FORMAT\\_bulk_question_upload__testinvite.txt")
